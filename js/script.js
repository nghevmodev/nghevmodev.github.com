$(document).ready(function() {
	...
	$items.isotope({
		itemSelector: '.site__case-study .item',
		layoutMode: 'fitRows',
		filter: '*'
	});

	$('.site__case-study .btns').on('click', 'button', function() {
		var filterValue = $(this).attr('data-filter');
		$items.isotope({ filter: filterValue });
	});

	$('.site__case-study .btns button').on('click', function() {
		$('.site__case-study .btns button').each(function() {
			$(this).removeClass('active');
		});
		$(this).toggleClass('active');
	});

	$('.site__case-study .btns button:first-child').addClass('active');
	// End Filter Case Study
});
